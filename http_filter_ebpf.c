...
struct key_t
{
    ...
    // could add ip addresses for use in the future
    // u32 src_ip; // source ip
    // u32 dst_ip; // destination ip
};
...
struct packet_t
{
    ...
    char payload[MAX_BUF_SIZE]; // payload
};

// BPF_HASH(http_events, struct key_t, struct packet_t, 1024);
BPF_HASH(http_events, u64, struct packet_t, 1024);
...
HTTP_MATCH:
    ...
    // prepare packet_t to send to userspace
    ...
    bpf_skb_load_bytes(skb, payload_offset, packet.payload, MAX_BUF_SIZE);

    // struct key_t key_t = {};
    // key_t.timestamp_ns = bpf_ktime_get_ns();
    // http_events.insert(&key_t, &packet);
    uint64_t timestamp_ns;
    timestamp_ns = bpf_ktime_get_ns();
    http_events.insert(&timestamp_ns, &packet);
