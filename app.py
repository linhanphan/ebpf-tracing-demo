...
def generate_random():
    ...
    time.sleep(
        wait_time / 1000.0
    )  # time.sleep expects seconds, so convert milliseconds to seconds

    # Check if the original random number is divisible by 5
    if random_number % 5 == 0:
        # Simulate a server error
        return (
            jsonify({"error": "Server encountered an error processing your request."}),
            500,
        )
    else:
...
if __name__ == "__main__":
    app.run(post="0.0.0.0", port=8080, debug=True)
